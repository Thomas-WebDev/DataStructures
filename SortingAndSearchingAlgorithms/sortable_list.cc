...
/* Post:  */
{
  for (int first_unsorted = 1; first_unsorted < List<Record>::count; first_unsorted++) {
    for (int position = 0; position <= List<Record>::count - 1; position++){
      if (List<Record>::entry[position] > List<Record>::entry[position + 1])
...
class Record. */
{
   ...
   *If low < j <= last_small then List<Record>::entry[j].key < pivot.
   *If last_small < j < i then List<Record>::entry[j].key >= pivot. */
